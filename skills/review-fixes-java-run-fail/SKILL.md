...
- Total metadata entry counts are not below the 25% severe-drop threshold, or the reduction is convincingly explained by a changed upstream API/runtime surface, or the new version reports zero dynamic-access calls and therefore needs no metadata to run natively.
