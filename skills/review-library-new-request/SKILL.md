...
12. **`--initialize-at-build-time` needs all four conditions.** A `build.gradle` adding `--initialize-at-build-time` under `graalvmNative` is permitted only under the bounded consumer-required exception, so require the PR to establish every condition: the library builds no native image without the flag, the requirement comes from the library or its transitive dependencies rather than the test, the flag names only the forcing types, and the shipped `reachability-metadata.json` carries a top-level `comment` naming the flags the consumer must add. Block when any condition is unevidenced, when the flag names a package or the library at large, when the `comment` is missing, or when the flag stands in place of a metadata entry — a failing native image is evidence of missing metadata until the first two conditions are demonstrated. The metadata bundle itself must still ship no `native-image.properties` or other build-time directive. §FS-test-contract.2.7
