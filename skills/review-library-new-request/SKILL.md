...
- For insufficient dynamic-access coverage: say that new-library PRs need dynamic-access coverage above 20% when there are dynamic-access calls to cover, and ask for stronger tests or refreshed coverage evidence. Do not raise this on a `chunked-dynamic-access` PR unless it reports zero covered dynamic-access calls.
