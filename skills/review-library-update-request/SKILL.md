...
- Do not accept newly added JVM toolchain, target, or release overrides when they move tests away from the repository's stable test JDK in `ci.json`. Explicit `JavaLanguageVersion.of(...)`, `jvmToolchain(...)`, `kotlinOptions.jvmTarget = "..."`, or `options.release = ...` settings are acceptable only when they match that stable test JDK. Reject old-JDK compatibility flags such as `-Djava.security.manager=allow`.
