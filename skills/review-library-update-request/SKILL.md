...
- Do not accept newly added JVM toolchain, target, or release overrides unless they match the repository's stable test JDK in `ci.json`. This includes setting `JavaLanguageVersion.of(...)`, `jvmToolchain(...)`, `kotlinOptions.jvmTarget = "..."`, or `options.release = ...` to a different JDK, and adding old-JDK compatibility flags such as `-Djava.security.manager=allow`.
