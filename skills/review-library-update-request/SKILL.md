...
6. Review test quality.
   ...
   - When the requested version reports zero dynamic-access calls, do not escalate test-depth concerns (a shallow test, or one exercising behavior outside the library's responsibility) to a blocking issue or `human-intervention`. The zero-call report gives too little information to judge metadata relevance, so depth-of-coverage objections carry little signal. The native execution gate and any issue-requested metadata gate still apply.
