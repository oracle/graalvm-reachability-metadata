...
- Do not accept newly added JVM toolchain, target, or release overrides when they move tests away from the TCK-resolved test JVM version. When a language plugin needs an explicit `JavaLanguageVersion.of(...)`, `jvmToolchain(...)`, `kotlinOptions.jvmTarget = ...`, or `options.release = ...` setting, it should use the resolved test JVM version instead of hardcoding a number. Reject old-JDK compatibility flags such as `-Djava.security.manager=allow`.
