...
4. Check dynamic-access coverage across versions.
   ...
   - A coverage percentage drop of more than 20 percentage points for the new version is blocking unless the PR explains why the reduction is expected. Percentage drops of 20 points or less, and any reduction in absolute `coveredCalls` or `totalCalls`, are not drops and must not be reported as regressions.
   - Do not apply the coverage percentage drop rule to a PR that carries the `chunked-dynamic-access` label, and treat that exemption as overriding the drop rule wherever it appears in this skill: the chunk covers only the classes in its own chunk while the percentage is measured against the whole library, so it cannot reflect the chunk's work. Block such a PR on coverage only when it reports zero covered dynamic-access calls while the report has calls to cover (§FS-contribution-contract.3).
