...
- A native build or run flag in `build.gradle` is not a repair. Permit it only when the PR evidences the uniform necessity rule: no test of any shape builds and runs without the flag, the necessity originates in the library or its transitive dependencies rather than the test, and the flag is the narrowest that resolves it. A failing native image is evidence of missing metadata until the first two are shown. §FS-test-contract.2.7
