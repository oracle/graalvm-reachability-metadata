...
| Skip gate | Strategy parameter `skip-native-test-verification` or environment variable `FORGE_SKIP_NATIVE_TEST_VERIFICATION=1` | Default **false**. When enabled, callers treat the native-test verification gate as passed without running JVM-agent metadata collection, native tracing, or Codex recovery. Use only for controlled debugging branches. |
