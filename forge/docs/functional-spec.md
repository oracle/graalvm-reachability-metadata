...
| **Workflow engine** | A registered state-machine-like workflow implementation among the core workflow objects in [ai_workflows/core/](../ai_workflows/core/), such as `basic_iterative` or `dynamic_access_iterative`. The legacy [ai_workflows/workflow_strategies/](../ai_workflows/workflow_strategies/) package is an import-only compatibility layer after the relocation by §ROADMAP-forge-ai-workflows-structure. The engine owns prompts, command execution, retries, transitions, and terminal status selection for one run. |
