...
# The pre-push review's findings record is expected publication output, not
# repository-level surgery, so it must never set the intervention flag.
...
class RepairAttempt:
    """Outcome of one bounded repair-agent run (§FS-local-branch-review)."""
...
def run_repair_agent(
    ...
    """Run one bounded repair agent and commit only what it changed.

    Shared by the failed-gate fixup of §FS-local-ci-equivalent-verification.2 and
    the review repair of §FS-local-branch-review: both hand a failure record to an
    agent, then keep the result only when it actually edited the tree.
    """
    ...
    try:
        ...
        return RepairAttempt(command, None, [], displayed_log_path, "timed out")

    if completed.returncode != 0:
        return RepairAttempt(
            command, None, [], displayed_log_path, f"exited with code {completed.returncode}",
        )
...
def write_verification_metrics(metrics_repo_path: str | None, result: LocalCIVerificationResult) -> None:
    """Record a verification result in the durable pending run metrics.

    Public so a caller that discards a verification run — the review repair that
    reverts a failed re-verification (§FS-local-branch-review) — can restore the
    result that actually describes the published tree.
    """
