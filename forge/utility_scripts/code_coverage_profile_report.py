...
# navigation: §CC-code-coverage-improvement.3.2, §CC-code-coverage-improvement-architecture.1.
...
#: thread the test does not control (§CC-code-coverage-improvement.3.2.1).
...
def library_owners(library_methods: set[str] | None) -> set[str] | None:
    ...
    (§CC-code-coverage-improvement.3.2).
    """
...
def _index_synthetic_lambdas(graph: CallGraph) -> None:
    ...
    a prompt (§CC-code-coverage-improvement.3.2.1).
    """
...
def _mark_dispatch_edges(
    ...
    elsewhere and handed in (§CC-code-coverage-improvement.3.2.1). Or its
...
def _closure_stats(
    ...
    agent can act on (§CC-code-coverage-improvement.3.2.1). A body JaCoCo never
...
def correlate(
    ...
    # resolved library jars actually declare (§CC-code-coverage-improvement.3.2).
