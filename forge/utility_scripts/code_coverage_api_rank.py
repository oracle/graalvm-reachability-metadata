...
over-approximating call graph are specified in §AR-code-coverage-improvement.4.1.1;
...
from utility_scripts.code_coverage_api_eligibility import (
    ApiRankError,
    TypeModel,
    eligible_targets,
    load_type_model,
...
#: Hard cap on prompt targets per pass, matching §AR-code-coverage-improvement.4.1.
...
def select_targets(
    ...
    (§AR-code-coverage-improvement.4.1.1). Reaching an entry statically is an
    ...
    while heap and len(selected) < limit:
        ...
        if exact == 0:
            ...
            # targets (§AR-code-coverage-improvement.4.1.1).
            break
        unlocked |= reach[node]
...
def rank(
    ...
    # as uncovered (§AR-code-coverage-improvement.4.1.1). Public API entries are
    ...
    # (§AR-code-coverage-improvement.4.1.2). An owner the extractor never
