...
# The public-API phase's exact JaCoCo correlation: §AR-code-coverage-improvement.4.1.
# JVM-only by design, with native metadata prepared once before PGO discovery rather
# than per iteration: §AR-code-coverage-improvement.3.
...
def correlate_jacoco(inventory: dict, jacoco_xml_paths: list[str]) -> dict:
    ...
    coverage (§AR-code-coverage-improvement.4.1).
    """
