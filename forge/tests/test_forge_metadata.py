...
class FinalizeSuccessfulIssueTests(unittest.TestCase):
    ...
    def test_fixture_java_fix_handoff_simulates_one_retry_safe_new_issue(self) -> None:
        with tempfile.TemporaryDirectory() as repo_path:
            metrics_path = os.path.join(repo_path, "metrics")
            os.makedirs(metrics_path)
            pending_metrics = {
                "timestamp": "2026-07-27T10:00:00.000000Z",
                "library": "org.example:lib:2.0.0",
                "status": "success",
            ...
            with open(
                    os.path.join(metrics_path, PENDING_METRICS_FILENAME),
                    "w",
                    encoding="utf-8",
            ) as pending_file:
                json.dump(pending_metrics, pending_file)
            claimed_issue = forge_metadata.ClaimedIssue(
                issue={"number": 1412},
                label=forge_metadata.LABEL_JAVAC_FAIL,
                item_id="project-item",
                base_reachability_metadata_path=repo_path,
                worktree_path=repo_path,
                scratch_metrics_repo_path=metrics_path,
                issue_coordinates="org.example:lib:2.0.0",
                current_coordinates="org.example:lib:1.0.0",
                new_version="2.0.0",
            )

            with patch.object(forge_metadata, "is_fixture_testing_enabled", return_value=True):
                forge_metadata.prepare_java_fix_coverage_follow_up(claimed_issue)
                forge_metadata.prepare_java_fix_coverage_follow_up(claimed_issue)
            ...
            with open(
                ...
                persisted = json.load(pending_file)

        expected_issue_number = (
            forge_metadata.FIXTURE_COVERAGE_FOLLOW_UP_ISSUE_OFFSET + 1412
        )
        self.assertEqual(
            persisted["dynamic_access_handoff"]["follow_up_issue_number"],
            expected_issue_number,
        )
        self.assertEqual(
            persisted["dynamic_access_handoff"]["follow_up_issue_url"],
            f"https://github.com/{forge_metadata.REPO}/issues/{expected_issue_number}",
        )


class UnblockedIssueMergeFollowUpTests(unittest.TestCase):
    def test_unblock_trailer_releases_parked_issue_to_todo(self) -> None:
        pull_request = {
            "number": 77,
            "body": "Repair complete.\n\nForge-Unblocks-Issue: #9999\n",
        }

        with patch.object(
                forge_metadata,
                "get_project_item_id",
                return_value="project-item-9999",
        ...
        ) as clear_assignees, patch.object(
                forge_metadata,
                "invalidate_issue_claim_cache_entry",
        ) as invalidate_cache:
            forge_metadata.apply_unblocked_issue_merge_follow_up(pull_request)

        get_item.assert_called_once_with(9999)
        set_status.assert_called_once_with("project-item-9999", forge_metadata.STATUS_TODO)
        clear_assignees.assert_called_once_with(9999)
        invalidate_cache.assert_called_once_with(9999)
...
class WorkflowStrategyDispatchTests(unittest.TestCase):
    def test_omitted_strategy_lets_java_run_driver_apply_default(self) -> None:
        with tempfile.TemporaryDirectory() as worktree_path:
            ...
            with patch.object(forge_metadata, "require_claimed_issue_worktree"), \
                    patch.object(forge_metadata, "run_library_preparation_preflight", return_value=None), \
                    patch.object(forge_metadata, "dynamic_access_chunk_class_threshold", return_value=7), \
            ...
            self.assertEqual(
                argv[argv.index("--dynamic-access-handoff-class-threshold") + 1],
                "7",
            )
    ...
    def test_omitted_strategy_lets_javac_driver_apply_default(self) -> None:
        with tempfile.TemporaryDirectory() as worktree_path:
            claimed_issue = forge_metadata.ClaimedIssue(
                issue={"number": 1413},
                label=forge_metadata.LABEL_JAVAC_FAIL,
            ...
            self.assertEqual(marker.strategy_name, forge_metadata.DEFAULT_JAVAC_STRATEGY)

    def test_library_update_repair_routes_do_not_enable_java_fix_handoff(self) -> None:
        claimed_issue = _claimed_issue(forge_metadata.LABEL_LIBRARY_UPDATE)

        for selected_driver in (
                forge_metadata.ROUTE_FIX_JAVAC,
                forge_metadata.ROUTE_FIX_JAVA_RUN,
        ):
            with self.subTest(selected_driver=selected_driver):
                route = forge_metadata.LibraryUpdateRoute(
                    selected_driver=selected_driver,
                    baseline_coordinates="org.example:lib:1.0.0",
                    new_version="2.0.0",
                )
