...
class JavaFailWorkflowProjectPrepTests(unittest.TestCase):
    def test_javac_metadata_index_update_promotes_newer_final_version(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                os.chdir(temp_dir)
                with patch("ai_workflows.drivers.java_fail_workspace.run_gradle_task") as run_gradle_task:
    ...
    def test_copy_and_prepare_project_dir_skips_same_source_and_destination(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                os.chdir(temp_dir)
                with patch("ai_workflows.drivers.java_fail_workspace.shutil.copytree") as copytree:
    ...
    def test_create_project_prep_checkpoint_allows_no_staged_changes(self) -> None:
        with patch("ai_workflows.drivers.java_fail_workspace.build_ai_branch_name", return_value="ai/test-branch"), \
                patch("ai_workflows.drivers.java_fail_workspace.delete_remote_branch_if_exists") as delete_remote, \
                patch("ai_workflows.drivers.java_fail_workspace.subprocess.run") as run, \
                patch(
                    "ai_workflows.drivers.java_fail_workspace.subprocess.check_output",
                    return_value="abc123\n",
                ) as check_output:
            run.side_effect = [
    ...
    def test_failed_workflow_reset_preserves_last_passing_candidate(self) -> None:
        with patch("ai_workflows.drivers.java_fail_workspace.subprocess.run") as run, \
                patch(
                    "ai_workflows.drivers.java_fail_workspace.subprocess.check_output",
                    return_value="candidate\n",
                ) as check_output, \
                patch("ai_workflows.drivers.java_fail_workspace.shutil.rmtree") as rmtree:
