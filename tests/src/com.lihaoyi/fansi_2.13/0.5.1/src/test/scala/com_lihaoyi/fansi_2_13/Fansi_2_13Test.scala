 ...
 */
package com_lihaoyi.fansi_2_13
...
class Fansi_2_13Test {
  ...
  def ansiParsingSupportsKnownEscapesResetAndErrorModes(): Unit = {
    ...
    assertTrue(thrown.getMessage.contains("Unknown ansi-escape [999m"))
  ...
  def concatenationJoiningSplittingAndSubstringsPreserveStyles(): Unit = {
    ...
    val joinedWithoutSeparator: Str = Str.join(Seq(red, blue))
    ...
    assertEquals("abcd", joinedWithoutSeparator.plainText)
  ...
  def selectiveResetAttributesClearOnlyMatchingStyleCategories(): Unit = {
    ...
    val categoryReset: Str = (Color.Reset ++ Back.Reset ++ Bold.Off ++ Underlined.Off ++ Reversed.Off)(styled)
    val foregroundAndDecoration: Str = (Color.Red ++ Bold.On)(Str("y"))
    val attrResetForegroundAndDecoration: Str = Attr.Reset(foregroundAndDecoration)
    ...
    assertEquals(0L, categoryReset.getColor(0))
    assertEquals("x", categoryReset.render)
    assertEquals(0L, attrResetForegroundAndDecoration.getColor(0))
    assertEquals("y", attrResetForegroundAndDecoration.render)
  ...
  def publicAttributeMetadataAndValidationAreAvailable(): Unit = {
    ...
    assertEquals(256, Color.Full.length)
    assertEquals(256, Back.Full.length)

    val trueByIndex: Attr = Color.True(0x0a0b0c)
    ...
    assertThrows(classOf[IllegalArgumentException], () => Color.True(1 << 25))
