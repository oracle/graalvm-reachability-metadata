...
public class Spring_boot_data_jdbcTest {
    ...
    @Test
    void autoConfigurationUsesCustomNamingStrategyForJdbcMapping() {
        this.contextRunner.withUserConfiguration(CustomNamingStrategyConfiguration.class)
                .withPropertyValues(dataSourceProperties("spring.data.jdbc.dialect=h2"))
                .run((context) -> {
                    assertThat(context).hasSingleBean(NamingStrategy.class);

                    JdbcTemplate jdbcTemplate = context.getBean(JdbcTemplate.class);
                    createNamedProductsTable(jdbcTemplate);

                    ProductRepository repository = context.getBean(ProductRepository.class);
                    ProductItem product = repository.save(new ProductItem("Keyboard"));

                    assertThat(product.getId()).isNotNull();
                    assertThat(repository.findById(product.getId())).hasValueSatisfying(
                            (savedProduct) -> assertThat(savedProduct.getDisplayName()).isEqualTo("Keyboard"));
                    assertThat(jdbcTemplate.queryForObject("SELECT \"TITLE\" FROM \"NAMED_PRODUCTS\" WHERE \"ID\" = ?",
                            String.class, product.getId())).isEqualTo("Keyboard");
                    jdbcTemplate.execute("SHUTDOWN");
                });
    }
    ...
    private static void createNamedProductsTable(JdbcTemplate jdbcTemplate) {
        jdbcTemplate.execute("""
                CREATE TABLE "NAMED_PRODUCTS" (
                    "ID" BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        NamingStrategy customNamingStrategy() {
            return new NamingStrategy() {
                ...
                public String getTableName(Class<?> type) {
                    if (ProductItem.class.equals(type)) {
                        return "named_products";
                    }
                ...
                public String getColumnName(RelationalPersistentProperty property) {
                    if (ProductItem.class.equals(property.getOwner().getType())
                            && "displayName".equals(property.getName())) {
                        return "title";
                    }
...
class ProductItem {
    ...
    ProductItem(String displayName) {
...
interface ProductRepository extends CrudRepository<ProductItem, Long> {
