...
final class Boopickle_2_13Test {
  @Test
  ...
  def usesCompositePicklerForExplicitPolymorphicProtocols(): Unit = {
    implicit val putValuePickler: Pickler[PutValue] = generatePickler[PutValue]
    implicit val deleteValuePickler: Pickler[DeleteValue] = generatePickler[DeleteValue]
    implicit val commandPickler: Pickler[WireCommand] = compositePickler[WireCommand]
  ...
  def usesTransformPicklersForDomainValueObjects(): Unit = {
    implicit val moneyPickler: Pickler[Money] = transformPickler((cents: Long) => Money(cents))(
      (money: Money) => money.cents
    )
  ...
  def picklesBuiltInAndCustomExceptionMessages(): Unit = {
    ...
    val builtIn: Throwable = roundTrip[Throwable](new IllegalArgumentException("bad argument"))(customExceptionPickler)
    val custom: Throwable = roundTrip[Throwable](new DomainFailure("domain rejected"))(customExceptionPickler)
  ...
  private def roundTrip[A](value: A)(implicit pickler: Pickler[A]): A = {
    val bytes: ByteBuffer = Pickle.intoBytes(value)(PickleState.pickleStateSpeed, pickler)
    Unpickle[A].fromBytes(bytes)(UnpickleState.unpickleStateSpeed)
  }
