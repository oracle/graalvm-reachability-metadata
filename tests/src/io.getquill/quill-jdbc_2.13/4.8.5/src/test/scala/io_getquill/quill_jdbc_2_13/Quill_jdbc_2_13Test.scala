...
class Quill_jdbc_2_13Test {
  ...
  @Test
  def returningGeneratedReadsDatabaseAssignedKeysFromJdbcInserts(): Unit = {
    val notes = quote(querySchema[GeneratedNote]("generated_notes"))
    createGeneratedNotesTable()

    val firstId: Long = context.run(
      notes.insert(_.body -> lift("first note")).returningGenerated(_.id)
    )
    val secondId: Long = context.run(
      notes.insert(_.body -> lift("second note")).returningGenerated(_.id)
    )

    assertThat(firstId).isGreaterThan(0L)
    assertThat(secondId).isGreaterThan(firstId)
    assertThat(context.run(notes.sortBy(_.id)).asJava).containsExactly(
      GeneratedNote(firstId, "first note"),
      GeneratedNote(secondId, "second note")
    )
  }
  ...
  private def createGeneratedNotesTable(): Unit = {
    requireSuccessfulProbe(
      """
...
object Quill_jdbc_2_13Test {
  ...
  final case class GeneratedNote(id: Long, body: String)
