 ...
 */
package io_getquill.quill_engine_3
...
class Quill_engine_3Test {
  ...
  @Test
  def quatsSupportLookupRenamingLeastUpperTypeAndShapeDescriptions(): Unit = {
    ...
    assertThat(nestedQuat.shortString)
      .isEqualTo("Contact(person:Person(id:V,firstName:V,age:V,active:BV),email:V)")

    val thrown: IllegalArgumentException = assertThrows(
      classOf[IllegalArgumentException],
      () => nestedQuat.lookup("missing", failNonExist = true)
    )
