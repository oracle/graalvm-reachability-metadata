...
public class UnsafeAtomicReferenceFieldUpdaterTest {
    ...
    @Test
    void createsUpdaterForVolatileReferenceField() throws Exception {
        AtomicReferenceFieldUpdater<UpdaterTarget, String> updater = createNettyReferenceFieldUpdater();
        if (updater == null) {
            updater = AtomicReferenceFieldUpdater.newUpdater(UpdaterTarget.class, String.class, "value");
        }

        UpdaterTarget target = new UpdaterTarget();
        Assertions.assertEquals("initial", updater.get(target));
        Assertions.assertTrue(updater.compareAndSet(target, "initial", "updated"));
        Assertions.assertEquals("updated", target.value);

        updater.set(target, "set");
        Assertions.assertEquals("set", updater.get(target));

        updater.lazySet(target, "lazy");
        Assertions.assertEquals("lazy", target.value);
        Assertions.assertTrue(updater.weakCompareAndSet(target, "lazy", "weak"));
        Assertions.assertEquals("weak", updater.get(target));
    }

    private static AtomicReferenceFieldUpdater<UpdaterTarget, String> createNettyReferenceFieldUpdater()
            throws Exception {
        URL isolatedLibraryLocation = findIsolatedLibraryLocation();
        if (isolatedLibraryLocation == null) {
            return PlatformDependent.<UpdaterTarget, String>newAtomicReferenceFieldUpdater(
                    UpdaterTarget.class,
                    "value");
        }
        ...
        try (SystemPropertiesRestorer ignored = new SystemPropertiesRestorer(
            ...
            try (NettyIsolatedClassLoader classLoader = new NettyIsolatedClassLoader(isolatedLibraryLocation)) {
                ...
                return invokeNewAtomicReferenceFieldUpdater(platformDependentClass);
            }
    ...
    private static AtomicReferenceFieldUpdater<UpdaterTarget, String> invokeNewAtomicReferenceFieldUpdater(
        ...
        return (AtomicReferenceFieldUpdater<UpdaterTarget, String>) method.invoke(null, UpdaterTarget.class, "value");
    }
