...
public class OpenwireV9BaseDataStreamMarshallerTest {

    @Test
    void unmarshalsExceptionResponseWithTightEncoding() throws Exception {
        ExceptionResponse decoded = roundTrip(createException("v9 tight root", "v9 tight cause"), true);

        assertDecodedException(decoded, "v9 tight root", "v9 tight cause");
    }

    @Test
    void unmarshalsExceptionResponseWithLooseEncoding() throws Exception {
        ExceptionResponse decoded = roundTrip(createException("v9 loose root", "v9 loose cause"), false);

        assertDecodedException(decoded, "v9 loose root", "v9 loose cause");
    }

    private ExceptionResponse roundTrip(IllegalStateException exception, boolean tightEncodingEnabled)
            throws Exception {
        OpenWireFormat wireFormat = new OpenWireFormat(9);
    ...
    private IllegalStateException createException(String message, String causeMessage) {
        ...
        cause.setStackTrace(new StackTraceElement[] {
            new StackTraceElement(
                "test.support.OpenwireV9Cause",
                "cause",
                "OpenwireV9BaseDataStreamMarshallerTest.java",
                41)
        });
    ...
    private void assertDecodedException(
        ...
        assertThat(rootStackTrace[0].getClassName()).isEqualTo("test.support.OpenwireV9Root");
        assertThat(rootStackTrace[0].getMethodName()).isEqualTo("root");
        assertThat(rootStackTrace[0].getFileName()).isEqualTo("OpenwireV9BaseDataStreamMarshallerTest.java");
        assertThat(rootStackTrace[0].getLineNumber()).isEqualTo(33);

        StackTraceElement[] causeStackTrace = cause.getStackTrace();
        assertThat(causeStackTrace).hasSize(1);
        assertThat(causeStackTrace[0].getClassName()).isEqualTo("test.support.OpenwireV9Cause");
        assertThat(causeStackTrace[0].getMethodName()).isEqualTo("cause");
        assertThat(causeStackTrace[0].getFileName()).isEqualTo("OpenwireV9BaseDataStreamMarshallerTest.java");
