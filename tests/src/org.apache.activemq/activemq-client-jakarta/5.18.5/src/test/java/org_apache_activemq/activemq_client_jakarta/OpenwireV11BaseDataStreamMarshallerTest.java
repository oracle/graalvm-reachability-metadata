...
public class OpenwireV11BaseDataStreamMarshallerTest {
    ...
    void unmarshalsExceptionResponseWithTightEncoding() throws Exception {
        ExceptionResponse decoded = roundTrip(createException("v11 tight root", "v11 tight cause"), true);

        assertDecodedException(decoded, "v11 tight root", "v11 tight cause");
    }

    @Test
    void unmarshalsExceptionResponseWithLooseEncoding() throws Exception {
        ExceptionResponse decoded = roundTrip(createException("v11 loose root", "v11 loose cause"), false);

        assertDecodedException(decoded, "v11 loose root", "v11 loose cause");
    }

    private ExceptionResponse roundTrip(IllegalStateException exception, boolean tightEncodingEnabled)
            throws Exception {
        OpenWireFormat wireFormat = new OpenWireFormat(11);
    ...
    private IllegalStateException createException(String message, String causeMessage) {
        ...
        cause.setStackTrace(new StackTraceElement[] {
            new StackTraceElement(
                "test.support.OpenwireV11Cause",
                "cause",
                "OpenwireV11BaseDataStreamMarshallerTest.java",
                41)
        });
    ...
    private void assertDecodedException(
        ...
        assertThat(rootStackTrace[0].getClassName()).isEqualTo("test.support.OpenwireV11Root");
        assertThat(rootStackTrace[0].getMethodName()).isEqualTo("root");
        assertThat(rootStackTrace[0].getFileName()).isEqualTo("OpenwireV11BaseDataStreamMarshallerTest.java");
        assertThat(rootStackTrace[0].getLineNumber()).isEqualTo(33);

        StackTraceElement[] causeStackTrace = cause.getStackTrace();
        assertThat(causeStackTrace).hasSize(1);
        assertThat(causeStackTrace[0].getClassName()).isEqualTo("test.support.OpenwireV11Cause");
        assertThat(causeStackTrace[0].getMethodName()).isEqualTo("cause");
        assertThat(causeStackTrace[0].getFileName()).isEqualTo("OpenwireV11BaseDataStreamMarshallerTest.java");
