...
public class UserAuthGSSAPIWithMICTest {
    private static final String SERVER_VERSION = "SSH-2.0-jsch-gssapi-coverage-server";
    ...
    private static final byte[] KRB5_OID = new byte[] {
            0x6, 0x9, 0x2a, (byte) 0x86, 0x48, (byte) 0x86, (byte) 0xf7,
            0x12, 0x1, 0x2, 0x2
    };

    @Test
    void gssApiAuthenticationLoadsConfiguredGssContext() throws Exception {
        Session session = new JSch().getSession("coverage", "example.test", 22);
        session.setSocketFactory(new GssApiResponseSocketFactory());
        configureGssApiHandshake(session);
    ...
    private static void configureGssApiHandshake(Session session) {
        ...
        session.setConfig("userauth.none", SkippingUserAuthNone.class.getName());
        session.setConfig("gssapi-with-mic.krb5", ThrowingGssContext.class.getName());
    }

    public static final class SkippingUserAuthNone extends UserAuthNone {
        @Override
    ...
    public static final class ThrowingGssContext implements GSSContext {
        public ThrowingGssContext() {
        }

        @Override
        public void create(String user, String host) throws JSchException {
            throw new JSchException("Synthetic GSS context stops after reflective construction");
        }
        ...
        @Override
        public byte[] init(byte[] token, int start, int length) {
            return new byte[0];
        }

        @Override
        public byte[] getMIC(byte[] message, int start, int length) {
            return new byte[0];
        }
    ...
    private static final class GssApiResponseSocketFactory implements SocketFactory {
        private final GssApiResponseTransport transport = new GssApiResponseTransport();
    ...
    private static final class GssApiResponseTransport {
        ...
        private GssApiResponseTransport() {
        ...
        private void processClientPacket(byte[] payload) {
            ...
            if (command == 20) {
                ...
                if (!newKeysQueued) {
                    ...
                    serverSequence++;
                    pendingInput.add(encryptedPacket(gssApiResponsePayload()));
                    newKeysQueued = true;
                }
            }
        }
        ...
        private byte[] ecdhReplyPayload(byte[] clientPoint) {
            try {
                ...
                initializeServerCipher(encodedSecret, exchangeHash);
        ...
        private void initializeServerCipher(byte[] sharedSecret, byte[] exchangeHash)
                throws GeneralSecurityException, IOException {
            byte[] iv = deriveKey(sharedSecret, exchangeHash, (byte) 'B');
            byte[] key = deriveKey(sharedSecret, exchangeHash, (byte) 'D');
            serverMacKey = deriveKey(sharedSecret, exchangeHash, (byte) 'F');
            serverCipher = Cipher.getInstance("AES/CTR/NoPadding");
            serverCipher.init(Cipher.ENCRYPT_MODE,
                    new SecretKeySpec(Arrays.copyOf(key, 16), "AES"),
                    new IvParameterSpec(Arrays.copyOf(iv, 16)));
        }

        private byte[] encryptedPacket(byte[] payload) {
            try {
                byte[] plainPacket = packet(payload);
                Mac mac = Mac.getInstance("HmacSHA256");
                mac.init(new SecretKeySpec(serverMacKey, "HmacSHA256"));
                mac.update(intBytes(serverSequence));
                mac.update(plainPacket);
                byte[] encryptedPacket = serverCipher.update(plainPacket);
    ...
    private static byte[] gssApiResponsePayload() {
        try {
            ByteArrayOutputStream payload = new ByteArrayOutputStream();
            payload.write(60);
            writeString(payload, KRB5_OID);
            return payload.toByteArray();
        } catch (IOException ex) {
            throw new IllegalStateException(ex);
        }
    ...
    private static byte[] deriveKey(byte[] sharedSecret, byte[] exchangeHash, byte discriminator)
            throws GeneralSecurityException, IOException {
        ByteArrayOutputStream data = new ByteArrayOutputStream();
        data.write(sharedSecret);
        data.write(exchangeHash);
        data.write(discriminator);
        data.write(exchangeHash);
        return MessageDigest.getInstance("SHA-256").digest(data.toByteArray());
    }
