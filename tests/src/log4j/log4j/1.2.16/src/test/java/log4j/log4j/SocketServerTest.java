...
public class SocketServerTest {
    ...
    @Test
    void initializingSocketServerInFreshClassLoaderResolvesItsOwnClassLiteral() throws Exception {
        try (URLClassLoader isolatedLoader = new URLClassLoader(isolatedClassPath(), ClassLoader.getPlatformClassLoader())) {
            Class<?> socketServerClass = Class.forName("org.apache.log4j.net.SocketServer", true, isolatedLoader);

            assertThat(socketServerClass.getName()).isEqualTo("org.apache.log4j.net.SocketServer");
            assertThat(socketServerClass.getClassLoader()).isSameAs(isolatedLoader);
        }
    }
