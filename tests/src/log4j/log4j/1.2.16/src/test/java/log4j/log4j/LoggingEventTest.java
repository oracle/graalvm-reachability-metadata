...
public class LoggingEventTest {
    ...
    void serializesAndDeserializesBuiltInLevels() throws IOException, ClassNotFoundException {
        LoggingEvent original = new LoggingEvent(
                ...
                Logger.getLogger(LoggingEventTest.class),
                Level.DEBUG,
                "built-in level",
                null);

        LoggingEvent restored = roundTrip(original);

        assertThat(restored.getLevel()).isSameAs(Level.DEBUG);
        assertThat(restored.getRenderedMessage()).isEqualTo("built-in level");
        assertThat(restored.getLoggerName()).isEqualTo(LoggingEventTest.class.getName());
    }
    ...
    void serializesAndDeserializesCustomLevelsUsingTheSubclassFactoryMethod() throws IOException, ClassNotFoundException {
        ...
        assertThat(restored.getLoggerName()).isEqualTo("custom-level-logger");
    }
    ...
    private static LoggingEvent roundTrip(LoggingEvent event) throws IOException, ClassNotFoundException {
        ...
        try (ObjectInputStream objectInput = new ObjectInputStream(new ByteArrayInputStream(output.toByteArray()))) {
            return (LoggingEvent) objectInput.readObject();
        }
    }
