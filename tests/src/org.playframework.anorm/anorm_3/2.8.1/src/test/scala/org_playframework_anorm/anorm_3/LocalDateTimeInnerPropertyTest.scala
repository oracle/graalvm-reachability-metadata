...
class LocalDateTimeInnerPropertyTest {
  @Test
  def serializesAndDeserializesMinuteOfHourProperty(): Unit = {
    val originalDateTime: LocalDateTime = new LocalDateTime(2020, 2, 29, 12, 34, 56, 789)
    val original: LocalDateTime.Property = originalDateTime.minuteOfHour()

    val restored: LocalDateTime.Property = deserialize(serialize(original))
  ...
  private def deserialize(serialized: Array[Byte]): LocalDateTime.Property = {
    val objectInput: ObjectInputStream = new ObjectInputStream(new ByteArrayInputStream(serialized))
    try {
      objectInput.readObject().asInstanceOf[LocalDateTime.Property]
    } finally {
      objectInput.close()
    }
  }
