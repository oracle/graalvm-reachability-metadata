...
public class CustomEnumDefinitionProviderTest {
    ...
    @Test
    void jsonValueAnnotatedEnumMethodDefinesStringEnumValues() {
        ...
        assertThat(enumValues(schema)).containsExactly("draft", "published");
    }

    @Test
    void jsonPropertyAnnotatedEnumConstantsDefineStringEnumValues() {
        JsonNode schema = generateSchema(JsonPropertyStatus.class, JacksonOption.FLATTENED_ENUMS_FROM_JSONPROPERTY);

        assertThat(schema.get("type").asText()).isEqualTo("string");
        assertThat(enumValues(schema)).containsExactly("awaiting-review", "accepted");
    }
