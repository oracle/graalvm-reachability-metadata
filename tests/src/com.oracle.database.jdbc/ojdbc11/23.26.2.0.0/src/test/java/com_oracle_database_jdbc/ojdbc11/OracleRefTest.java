...
public class OracleRefTest {
    @Test
    void convertsToAnOracleDataFactory() throws Exception {
        OracleRef ref = new OracleRef("APP.TEST_REF", new DetachedConnection(), new byte[] {1, 3, 5});
    ...
    void retainsReferenceIdentityAcrossSerialization() throws Exception {
        OracleRef original = new OracleRef("APP.TEST_REF", new DetachedConnection(), new byte[] {2, 4, 6});

        OracleRef restored = roundTrip(original);

        assertThat(restored.getBaseTypeName()).isEqualTo("APP.TEST_REF");
        assertThat(restored.shareBytes()).containsExactly(2, 4, 6);
    }

    private static OracleRef roundTrip(OracleRef ref) throws Exception {
        ByteArrayOutputStream bytes = new ByteArrayOutputStream();
        try (ObjectOutputStream output = new ObjectOutputStream(bytes)) {
            output.writeObject(ref);
        }
        try (ObjectInputStream input = new ObjectInputStream(new ByteArrayInputStream(bytes.toByteArray()))) {
            return (OracleRef) input.readObject();
        }
    }
