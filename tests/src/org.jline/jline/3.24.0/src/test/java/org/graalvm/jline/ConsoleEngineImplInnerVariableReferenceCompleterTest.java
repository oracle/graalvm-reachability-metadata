...
public class ConsoleEngineImplInnerVariableReferenceCompleterTest {
    ...
    @Test
    public void completesPublicGetterNamesForObjectVariable() throws Exception {
        InMemoryScriptEngine scriptEngine = new InMemoryScriptEngine();
        scriptEngine.put("account", new Account("Ada"));
        Path configDirectory = Files.createTempDirectory("jline-console-completion");
        ConsoleEngineImpl consoleEngine = new ConsoleEngineImpl(
                scriptEngine,
                new NoOpPrinter(),
                configDirectory::toAbsolutePath,
                new ConfigurationPath(configDirectory, configDirectory));
        ...
        try (Terminal terminal = new DumbTerminal(
            ...
            LineReader reader = LineReaderBuilder.builder()
                    ...
                    .build();
            SystemCompleter completer = consoleEngine.compileCompleters();
            completer.compile();

            List<Candidate> candidates = complete(completer, reader, "slurp ${account}.");
            List<String> values = candidates.stream()
                    .map(Candidate::value)
                    .collect(Collectors.toList());

            assertTrue(values.contains("${account}.class"));
            assertTrue(values.contains("${account}.displayName"));
        }
    }
    ...
    private static class InMemoryScriptEngine implements ScriptEngine {
