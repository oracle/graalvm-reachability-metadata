...
public class SecurityActionsAnonymous2Test {
    private static final String BRIDGE_CLASS_NAME = "javassist.util.proxy.SecurityActionsAnonymous2CoverageBridge";
    private static final String SECURITY_ACTION_CLASS_NAME = "javassist.util.proxy.SecurityActions$2";

    @Test
    void privilegedActionReadsDeclaredConstructors() throws Exception {
        try {
            Supplier<?> supplier = (Supplier<?>) createBridgeClass().newInstance();
            Constructor<?>[] constructors = (Constructor<?>[]) supplier.get();
    ...
    private static Class<?> createBridgeClass() throws Exception {
        ...
        return classPool.toClass(
                bridgeClass,
                SecurityActionsAnonymous2Test.class.getClassLoader(),
                SecurityActionsAnonymous2Test.class.getProtectionDomain());
    }
