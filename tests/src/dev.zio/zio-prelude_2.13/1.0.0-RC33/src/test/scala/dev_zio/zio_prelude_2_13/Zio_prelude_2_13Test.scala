 ...
 */
package dev_zio.zio_prelude_2_13
...
class Zio_prelude_2_13Test {
  ...
  @Test
  def nonEmptySortedCollectionsPreserveOrderingAndNonEmptyConversions(): Unit = {
    val sortedSet: NonEmptySortedSet[Int] = NonEmptySortedSet(3, 1, 2, 2)
    val expandedSet: NonEmptySortedSet[Int] = sortedSet ++ List(5, 4)

    assertEquals(SortedSet(1, 2, 3), sortedSet.toSet)
    assertEquals((1, SortedSet(2, 3)), sortedSet.peel)
    assertEquals(Some(SortedSet(2, 3)), sortedSet.tailNonEmpty.map(_.toSet))
    assertEquals(SortedSet(1, 2, 3, 4, 5), expandedSet.toSet)
    assertEquals(SortedSet(2, 3), sortedSet.remove(1))
    assertEquals(Some(SortedSet(2, 3)), sortedSet.removeNonEmpty(1).map(_.toSet))
    assertEquals(None, NonEmptySortedSet.single(1).removeNonEmpty(1))
    assertEquals(Some(SortedSet(1, 2, 3)), NonEmptySortedSet.fromIterableOption(List(3, 1, 2)).map(_.toSet))
    assertTrue(NonEmptySortedSet.fromIterableOption(List.empty[Int]).isEmpty)

    val sortedMap: NonEmptySortedMap[String, Int] =
      NonEmptySortedMap.single("b" -> 2) ++ List("a" -> 1, "c" -> 3, "b" -> 4)

    assertEquals(SortedMap("a" -> 1, "b" -> 4, "c" -> 3), sortedMap.toMap)
    assertEquals(("a" -> 1, SortedMap("b" -> 4, "c" -> 3)), sortedMap.peel)
    assertEquals(SortedSet("a", "b", "c"), sortedMap.keySet.toSet)
    assertEquals(List(1, 4, 3), sortedMap.values.toList)
    assertEquals(SortedMap("a" -> "1 items", "b" -> "4 items", "c" -> "3 items"), sortedMap.mapValues(count => s"$count items").toMap)
    assertEquals(SortedMap("b" -> 4, "c" -> 3), sortedMap.remove("a"))
    assertTrue(sortedMap.tailNonEmpty.isDefined)
    assertTrue(NonEmptySortedMap.fromMapOption(SortedMap.empty[String, Int]).isEmpty)
    assertEquals(Some(NonEmptySortedMap.single("only" -> 1)), NonEmptySortedMap.fromMapOption(SortedMap("only" -> 1)))
  }
