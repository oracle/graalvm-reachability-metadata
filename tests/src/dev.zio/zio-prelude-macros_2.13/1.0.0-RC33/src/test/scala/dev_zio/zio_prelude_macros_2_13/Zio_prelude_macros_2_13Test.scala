 ...
 */
package dev_zio.zio_prelude_macros_2_13
...
class Zio_prelude_macros_2_13Test {
  @Test
  ...
  def stringAssertionsCoverCaseSensitivityLengthAndRegexEntryPoints(): Unit = {
    ...
    assertFails(
      Assertion.hasLength(Assertion.between(3, 5)),
      "toolong",
      "toolong did not satisfy hasLength(between(3, 5))"
    )
    ...
    assertPasses(Assertion.matches("[a-z]+-[0-9]+"), "build-33")
    assertFails(
      Assertion.matches("[a-z]+-[0-9]+"),
      "build-RC",
      "build-RC did not satisfy matches([a-z]+-[0-9]+)"
    )
  ...
  def regexDslCompilesComposablePatternsAndCharacterClasses(): Unit = {
    val animalCode: Regex =
      Regex.start ~ (Regex.literal("cat") | Regex.literal("dog")) ~ Regex.digit.between(2, 3) ~ Regex.end
    ...
    assertFails(
      Assertion.matches(animalCode),
      "bird12",
      "bird12 did not satisfy matches(^(cat|dog)(\\d){2,3}$)"
    )
  ...
  def buildInfoExposesPublishedModuleMetadataWithoutPinningTheArtifactVersion(): Unit = {
    ...
    assertThat(BuildInfo.scalaVersion).startsWith("2.13.")
  ...
  private def assertFails[A](assertion: Assertion[A], value: A, expectedMessages: String*): Unit = {
    ...
    assertThat(error.toNel(value.toString).asJava).containsExactly(expectedMessages: _*)
  }
