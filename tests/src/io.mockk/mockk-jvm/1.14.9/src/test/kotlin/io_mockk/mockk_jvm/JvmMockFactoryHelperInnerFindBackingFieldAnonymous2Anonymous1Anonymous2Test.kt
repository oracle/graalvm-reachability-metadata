...
public class JvmMockFactoryHelperInnerFindBackingFieldAnonymous2Anonymous1Anonymous2Test {
    @Test
    fun invocationProviderWritesKotlinPropertyBackingField(): Unit {
        val target: TargetWithMutableBackingField = TargetWithMutableBackingField("initial value")
        val getter: Method = TargetWithMutableBackingField::class.java.getDeclaredMethod("getMessage")
        val stub: CapturingStub = CapturingStub { fieldValueProvider ->
            val fieldValue: BackingFieldValue = fieldValueProvider()
                ?: error("Kotlin property getter should expose a writable backing field")
            fieldValue.setter("updated value")
        }

        JvmMockFactoryHelper.mockHandler(stub).invocation(
            target,
            getter,
        ...
        assertThat(target.message).isEqualTo("updated value")
    }

    private class TargetWithMutableBackingField(
        var message: String,
    )
    ...
    private class CapturingStub(
        ...
        override val type: KClass<*> = TargetWithMutableBackingField::class
