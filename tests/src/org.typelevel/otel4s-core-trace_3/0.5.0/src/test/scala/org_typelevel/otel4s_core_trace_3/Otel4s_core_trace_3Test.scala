...
class Otel4s_core_trace_3Test {
  ...
  def attributesAndEnumerationsExposeTypedValuesHashingAndShowInstances(): Unit = {
    ...
    assertSame(AttributeType.StringSeq, tagsKey.`type`)
  ...
  private final class RecordingSpanBuilder(operations: ListBuffer[String]) extends SpanBuilder[Id] {
    ...
    def addLink(spanContext: SpanContext, attributes: immutable.Iterable[Attribute[_]]): SpanBuilder[Id] = {
      operations += "addLink"
      this
    }
  ...
  private final class RecordingSpanBackend(override val context: SpanContext) extends Span.Backend[Id] {
    ...
    def addEvent(name: String, attributes: immutable.Iterable[Attribute[_]]): Id[Unit] = {
      operations += s"addEvent:$name:${attributes.size}"
      ()
    }

    def addEvent(name: String, timestamp: FiniteDuration, attributes: immutable.Iterable[Attribute[_]]): Id[Unit] = {
      operations += s"addEventAt:$name:${timestamp.toMillis}:${attributes.size}"
      ()
    }

    def recordException(throwable: Throwable, attributes: immutable.Iterable[Attribute[_]]): Id[Unit] = {
