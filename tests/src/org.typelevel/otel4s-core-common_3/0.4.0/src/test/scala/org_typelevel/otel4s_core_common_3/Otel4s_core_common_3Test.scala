...
class Otel4s_core_common_3Test {
  ...
  def attributesKeepNamesTypesValuesAndCatsInstances(): Unit = {
    ...
    val stringListKey: AttributeKey[List[String]] = AttributeKey.stringList("http.route")
    val booleanListKey: AttributeKey[List[Boolean]] = AttributeKey.booleanList("flags")
    val longListKey: AttributeKey[List[Long]] = AttributeKey.longList("retry.counts")
    val doubleListKey: AttributeKey[List[Double]] = AttributeKey.doubleList("bucket.bounds")
    ...
    assertEquals(AttributeType.StringList, stringListKey.`type`)
    assertEquals(AttributeType.BooleanList, booleanListKey.`type`)
    assertEquals(AttributeType.LongList, longListKey.`type`)
    assertEquals(AttributeType.DoubleList, doubleListKey.`type`)

    val directAttribute: Attribute[String] = stringKey("GET")
    val selectedAttribute: Attribute[String] = Attribute("http.method", "GET")
    val listAttribute: Attribute[List[Long]] = Attribute("retry.counts", List(1L, 2L, 3L))

    assertEquals(selectedAttribute, directAttribute)
    assertEquals(List(1L, 2L, 3L), listAttribute.value)
