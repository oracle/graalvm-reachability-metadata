...
public class SecuritySupportTest {
    ...
    private static final String RESOURCE_CONTENT = "resource loaded through explicit class loader";
    private static final String SYSTEM_RESOURCE_NAME = "java/lang/String.class";
    ...
    void getResourceAsStreamUsesSystemResourcesWhenClassLoaderIsNull() throws Exception {
        try {
            try (InputStream stream = getResourceAsStream(null, SYSTEM_RESOURCE_NAME)) {
                assertThat(stream).isNotNull();
                assertThat(stream.readNBytes(4))
                        .containsExactly((byte) 0xCA, (byte) 0xFE, (byte) 0xBA, (byte) 0xBE);
            }
        } catch (Error error) {
            if (!NativeImageSupport.isUnsupportedFeatureError(error)) {
                throw error;
