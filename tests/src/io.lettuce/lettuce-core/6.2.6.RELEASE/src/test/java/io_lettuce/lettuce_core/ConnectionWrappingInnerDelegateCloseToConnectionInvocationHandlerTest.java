...
public class ConnectionWrappingInnerDelegateCloseToConnectionInvocationHandlerTest {
    @Test
    void asyncApiDelegatesCommandsThroughPooledConnection() throws Exception {
        try (Lettuce_coreTest.FakeRedisServer server = new Lettuce_coreTest.FakeRedisServer()) {
            ...
            try {
                ...
                RedisFuture<String> ping = commands.ping();

                assertThat(ping.get(LettuceTestSupport.TIMEOUT.toSeconds(), TimeUnit.SECONDS)).isEqualTo("PONG");
                assertThat(commands.getStatefulConnection()).isSameAs(connection);
                connection.closeAsync().get(LettuceTestSupport.TIMEOUT.toSeconds(), TimeUnit.SECONDS);
