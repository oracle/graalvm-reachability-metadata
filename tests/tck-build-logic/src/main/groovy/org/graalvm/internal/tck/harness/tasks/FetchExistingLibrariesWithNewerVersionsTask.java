...
/**
 * Lists coordinates (group:artifact:newerVersion) for existing libraries that have newer versions
 * than the latest tested version, excluding infrastructure tests and skipped versions.
 * Prints a JSON array of objects: [{"name":"group:artifact","versions":[...]}]
 */
...
public abstract class FetchExistingLibrariesWithNewerVersionsTask extends DefaultTask {
     ...
     * alpha, beta, rc, cr, m<num>, ea, b<num>, preview, or numeric suffixes (case-insensitive).
     */
    private static final Pattern VERSION_PATTERN = Pattern.compile("(?i)^(\\d+(?:\\.\\d+)*)(?:\\.Final)?(?:[-.](alpha\\d*|beta\\d*|rc\\d*|cr\\d*|m\\d+|ea\\d*|b\\d+|\\d+|preview)(?:[-.].*)?)?$");

    @TaskAction
    public void action() {
        // get all existing libraries (group:artifact)
        Set<String> libraries = new LinkedHashSet<>();
        for (String coord : getAllLibraryCoordinates().get()) {
            int idx = coord.lastIndexOf(':');
            if (idx > 0) {
                libraries.add(coord.substring(0, idx));
            }
        }
        ...
        for (String libraryName : libraries) {
            if (INFRASTRUCTURE_TESTS.stream().anyMatch(libraryName::startsWith)) {
                continue;
            }
            String latest = getLatestLibraryVersion(libraryName);
            List<String> versions = getNewerVersionsFor(libraryName, latest);
            List<String> skipped = getSkippedVersions(libraryName);

            // filter out skipped versions
            versions.removeIf(skipped::contains);
        ...
        for (String coord : newerVersions) {
            ...
            if (parts.length >= 3) {
                ...
                map.computeIfAbsent(key, k -> new ArrayList<>()).add(parts[2]);
            }
        }
    ...
    static List<String> getNewerVersionsFor(String library, String startingVersion) {
        ...
        String url = baseUrl + "/" + group + "/" + artifact + "/maven-metadata.xml";
        String data;
        try {
            data = new String(new URL(url).openStream().readAllBytes(), StandardCharsets.UTF_8);
        } catch (IOException e) {
            throw new RuntimeException("Failed to read maven-metadata.xml for " + library + ": " + e.getMessage(), e);
        }
     ...
     * For provided Maven coordinates (groupId:artifactId), reads metadata/<groupId>/<artifactId>/index.json
