 ...
 * Reuses the same unified resolution as {@link CoordinatesAwareTask} and enforces a single result.
 */
@SuppressWarnings("unused")
public abstract class SingleCoordinateTask extends CoordinatesAwareTask {

    /**
     * Resolves to exactly one coordinate or fails with a helpful error.
     */
    protected String resolveSingleCoordinate() {
        java.util.List<String> coords = resolveCoordinates();
        if (coords.isEmpty()) {
