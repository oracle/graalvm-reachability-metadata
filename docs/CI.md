...
- [.github/workflows/publish-scheduled-coverage.yml](../.github/workflows/publish-scheduled-coverage.yml): scheduled/manual workflow that generates repository coverage badges, split execution metrics, and the coverage dashboard from exploded stats files under [stats/](../stats/) and repository [metadata/**/index.json](../metadata/), then force-pushes the published artifacts to the `stats/coverage` branch. The published branch only keeps `latest/badges.json`, `latest/execution-metrics/**/metrics.json`, `latest/metrics-over-time.svg`, and `history/history.json`.
