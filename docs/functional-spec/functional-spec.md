...
Forge's requirements are specified in [forge/docs/functional-spec/functional-spec.md](../../forge/docs/functional-spec/functional-spec.md).
...
§forge/AR-forge-orchestration); the bulk-update PR is reviewed by the
...
that `run-consecutive-tests.sh` isolated. When the library's unique
`latest: true` index entry also sets `high-priority: true`, the issue additionally
receives the `high-priority` label. `fails-native-image-build` issues are
...
- [forge/docs/functional-spec/functional-spec.md](../../forge/docs/functional-spec/functional-spec.md) — Forge functional requirements and workflow contracts.
